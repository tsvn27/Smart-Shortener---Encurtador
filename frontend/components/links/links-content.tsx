...
import { Search, Filter, Grid3X3, List, PlusCircle, X, Link2, Loader2, ChevronLeft, ChevronRight, ArrowUpDown, ArrowUp, ArrowDown } from "lucide-react"
...
type SortBy = "createdAt" | "totalClicks" | "clicksToday"
...
export function LinksContent() {
  ...
  const loadLinks = useCallback(async () => {
    setIsLoading(true)
    try {
      const res = await api.getLinks(ITEMS_PER_PAGE, (currentPage - 1) * ITEMS_PER_PAGE, {
      ...
      if (res.data.length === ITEMS_PER_PAGE) {
        setTotalLinks(Math.max(totalLinks, currentPage * ITEMS_PER_PAGE + 1))
      } else {
        setTotalLinks((currentPage - 1) * ITEMS_PER_PAGE + res.data.length)
      }
    } catch (error) {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-fade-in">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">{totalLinks} links criados</p>
        </div>
          ...
          <div className="flex gap-1">
            <button
              ...
              className={cn(
                ...
                sortBy === 'createdAt' ? "bg-white/[0.08] text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-white/[0.04]"
              )}
            >
              Data
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition-all",
                sortBy === 'totalClicks' ? "bg-white/[0.08] text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-white/[0.04]"
              )}
            >
              Cliques
            ...
            <button
              onClick={() => toggleSort('clicksToday')}
              className={cn(
                "flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition-all",
                sortBy === 'clicksToday' ? "bg-white/[0.08] text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-white/[0.04]"
              )}
            >
              Hoje
              <SortIcon field="clicksToday" />
            </button>
          </div>
        </div>
      </div>
      ...
      {links.length === 0 && !isLoading ? (
        <div className="glass-card rounded-2xl p-16 text-center animate-fade-in" style={{ animationDelay: "100ms" }}>
          ...
          <h3 className="text-xl font-semibold text-foreground mb-2">
            {debouncedSearch || statusFilter !== 'all' ? "Nenhum link encontrado" : "Nenhum link criado"}
          </h3>
